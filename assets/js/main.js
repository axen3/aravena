...
window.loadHeaderLinks = async function () {
    ...
    // Static Home Link
    const staticHomeLink =
        '<a href="/home" data-link><i class="fas fa-home"></i> Home</a>';
...
window.loadHome = async function (selectedCategory = null) {
    ...
    if (selectedCategory) {
        ...
        document.querySelector("h2")
            ? (document.querySelector("h2").textContent = "All Products")
            : null;
    }
        ...
        const filterHTML = `<div id="category-filters" class="category-filters">
            ${categories
                .map(cat => {
                    ...
                    return `
                    <a href="${path}" data-link class="category-btn ${
...
window.loadProduct = async function (id) {
    ...
    if (!product) {
        document.getElementById("app").innerHTML =
            "<h2 style='text-align:center;padding:4rem;'>Product not found 😢</h2>";
        return;
    }
    ...
    if (product.colors && product.colors.length > 0) {
        colorDiv.innerHTML = product.colors
            .map(
                (c, i) => `
            <div class="color-swatch ${
                i === 0 ? "active" : ""
            }" style="background:${c.hex}" data-color="${c.name}" title="${
                c.name
            }"></div>
        `
            )
    ...
    descDiv.innerHTML = (product.descriptionBlocks || [])
        .map(block => {
    ...
    const requiredFields = [
        {
            id: "name",
            minLength: 2,
            msg: "Full name is required (min 2 characters)."
        },
        {
            id: "phone",
            type: "phone",
            msg: "Phone must be 10 digits, starting with 0."
        },
        {
            id: "address",
            minLength: 10,
            msg: "Full address is required (min 10 characters)."
        },
...
if (form && !form.dataset.bound) {
    ...
    form.addEventListener("submit", async e => {
        ...
        formData.append("orderId", "WY-" + Date.now().toString().slice(-6));
        ...
        try {
            const res = await fetch(
                "https://script.google.com/macros/s/AKfycbzG8oCEKdqt678i9bYYnL22aIQEO42M09pYgGiUgIYU6imas3sjA03DXNST6oEKsfKV/exec",
        ...
        if (response.ok) {
            ...
            if (formWrapper && successMessage) {
                ...
                if (newSuccessMessage) {
                    ...
                    setTimeout(() => {
                        newSuccessMessage.style.transition =
                            "opacity 0.5s ease";
